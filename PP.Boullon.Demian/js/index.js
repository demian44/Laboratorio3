...
var callBack = function() {
	//Se pasa por parametro a httpReq.
	...
	if (httpReq.readyState == 4) {
		...
		if (httpReq.status == 200) {
			...
			notas.forEach(nota => {
				if (nota.nota < 4) body += "<tr style='color:red'>";
				else body += '<tr>';
				body +=
					'<td>' +
					nota.legajo +
					...
					nota.nota +
					'</td>';
				if (user.type == 'Admin') body += "<td><a onclick = 'editar(" + i + ")' href='#'>Editar </a></td>";
		...
		} else if (httpReq.status == 500) alert('ocurrió un error en el servidor.');
		else alert('Error: ' + httpReq.status);
	}
...
function armarCabecera(type) {
	var cabecera = '<tr><th>Legajo\
     ...
     <th>Materia</th>\
        <th> Nota  </th>';
	if (type == 'Admin') cabecera += '<th>Accion</th>';
	cabecera += '</tr >';
